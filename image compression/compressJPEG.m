function  [h, d, cb, cr]=compressJPEG(imagematrix,dgree_of_compression,blocksize)
...
% in huffman coding.  cb,and cr are the downsampled chrominance and
% luminance components
%*************************************************************************
%tracking folps: the global variables were used to count floating point
%operations(FLOPs). I have commented all the FLOP calculation bits of code
...
% check if image is greyscale or RGB and handle accordingly
if length(size(imagematrix))==3
    [x, y, ~]= size (imagematrix); 
    %for rgb convert to ycbcr and downsample
    [Y,cb,cr] = RGB_to_YCbCr(imagematrix,blocksize);
    imagematrix = Y;
else
    [x,y]= size (imagematrix);
    cb= nan; cr= nan;
...
% save the compressed image files to desk
fileID = fopen('compressed_outputs/compressed.bin','w');
fileid= fopen('compressed_outputs/huff_dict.bin','w');
fid= fopen('compressed_outputs/CbCr.bin','w');
fwrite(fid,cat(3,cb,cr));
fwrite(fileID,num_h);
