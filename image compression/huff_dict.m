...
i=n-1;
rws=find(track_done(:,i)==10);
codeword(rws)=strcat(codeword(rws),'1');
rws=find(track_done(:,i)==11);
codeword(rws)=strcat(codeword(rws),'0');
for i=n-2:-1:1
    r11=find(track_done(:,i)==11);
    r10=track_done(:,i)==10;
    codeword(r10)=strcat(codeword(r11),'1');
    codeword(r11)=strcat(codeword(r11),'0');
end
for i=1:length(p)
